...
ln ln_multiply(ln i,ln j,res_type restype)
{
	...
	//分配结果节点,节点数可以直接算出来
	k=ln_creat(ln_cell_num(i)+ln_cell_num(j));
	if(k==NULL)
	{
		fprintf(stderr,"[%s %d] %s error,reason: ln_creat fail\n",__FILE__,__LINE__,__FUNCTION__);
		return NULL;	
...
ln ln_divide_int(ln a,int b,int precision,divide_mode mode,res_type restype)
{
	...
	while(1)
	{
		...
		if(z==c->lsd) //增加节点
		{
			if(ln_addcell(c,INIT_SIZE) !=LN_SUCC)
			{
				fprintf(stderr,"[%s %d] %s error,reason: ln_addcell fail\n",__FILE__,__LINE__,__FUNCTION__);
				return NULL;
			}
...
ln ln_divide(ln a,ln b,int precision,divide_mode mode,res_type restype)
{
	...
	if(x_cell_num<y_cell_num)
		ln_adjustpower(x,(x_cell_num-y_cell_num)*DIGIT_NUM);
	if(y_cell_num<x_cell_num)
		ln_adjustpower(y,(y_cell_num-x_cell_num)*DIGIT_NUM);
	//算出初始精度
	...
	while(1)
	{
		...
		{
			if(ln_addcell(x,INIT_SIZE) !=LN_SUCC)
			{
				fprintf(stderr,"[%s %d] %s error,reason: ln_addcell fail\n",__FILE__,__LINE__,__FUNCTION__);
				return NULL;
			}
		...
		if(k==z->lsd)
		{
			if(ln_addcell(z,INIT_SIZE) !=LN_SUCC)
			{
				fprintf(stderr,"[%s %d] %s error,reason: ln_addcell fail\n",__FILE__,__LINE__,__FUNCTION__);
				return NULL;
			}
