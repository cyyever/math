...
public:
  ...
  */
  bool diffrent_sign(const my_int &rhs) const { return sign != rhs.sign; }
  int compare(const my_int &rhs) const;
  uint64_t digit_num() const;

  bool is_odd() const { return my_digit_list.back() & 1; }
...
private:
  ...
  bool is_zero() const { return my_digit_list.back() == 0; }
